...
def main():
    ...
    #model = ItemBasedRecommender(dataset, ignore_zero_rating=True)
    #model = UserBasedRecommender(dataset, ignore_zero_rating=True)
    #model = SVDRecommender(dataset)
    model = BiasSVDRecommender(dataset, use_global_mean=False, use_bias=True)
    #model = SVDPlusUserBasedRecommender(dataset, ignore_zero_rating=True)
    args = {}
    #args['model_parameters'] = {'similarity': 'cosine', 'K': 30, 'rebuild': False}
    args['model_parameters'] = {'dim': 28, 'rebuild': True}
    args['N'] = 700
    args['threshold'] = 3
