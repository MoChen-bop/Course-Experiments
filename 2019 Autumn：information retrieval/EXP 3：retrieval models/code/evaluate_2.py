...
def extract_query_from_file(query_file_path, gold_items, corpus):
	query_items = {}
	...
	for item in TOP_PATTERN.findall(lines):
		...
		docID = list(gold_items[topicID].keys())[0]
		for document in corpus.documents:
			if int(document.docID) == int(docID):
...
parser.add_argument('--output_path', required=False, 
					default='G:\\dataset\\corpus\\OHSUMED\\EvaluateReasult\\ohsu-87-2')
parser.add_argument('--gold_file_path', required=False, 
					default='G:\\dataset\\corpus\\OHSUMED\\ohsu-trec\\trec9-train\\qrels.ohsu.batch.87')
parser.add_argument('--query_file_path', required=False, 
					default='G:\\dataset\\corpus\\OHSUMED\\ohsu-trec\\trec9-train\\query.ohsu.1-63')
...
query_items = extract_query_from_file(query_file_path, gold_items, corpus)
...
evaluate_model(tf_model, "TF(normalized)", normalized=True)

print('evaluating tf-idf model...')
args = 'log'
tf_idf_model = TF_IDFModel(corpus, indexer, args)
evaluate_model(tf_idf_model, "TF-IDF(normalized)", normalized=True)
'''
