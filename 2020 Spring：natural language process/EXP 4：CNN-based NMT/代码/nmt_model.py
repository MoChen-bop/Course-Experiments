...
from model_embeddings import ModelEmbeddings, ContexAwareEmbeddings, ContexAwareEmbeddings_LSTM
from highway import SelectiveConnect
...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        if self.multi_encoder:
            enc_hiddens, dec_init_state = self.multi_encode(source_padded_chars, source_lengths)
        else:
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = nn.utils.rnn.pack_padded_sequence(input=X, lengths=source_lengths, batch_first=False)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens, _ = nn.utils.rnn.pad_packed_sequence(enc_hiddens, batch_first=False)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)

        last_hidden = last_hidden.permute(1, 0, 2).reshape(-1, 2 * self.hidden_size)
        init_decoder_hidden = self.h_projection(last_hidden)
        last_cell = last_cell.permute(1, 0, 2).reshape(-1, 2 * self.hidden_size)
    ...
    def multi_encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        """ Apply the encoder to source sentences to obtain encoder hidden states.
            Additionally, take the final states of the encoder and project them to obtain initial states for decoder.
        @param source_padded (Tensor): Tensor of padded source sentences with shape (src_len, b, max_word_length), where
        ...
        ### Except replace "self.model_embeddings.source" with "self.model_embeddings_source"
        # layer 1
        X = self.model_embeddings_source(source_padded) # sentence_L x batch_size x word_embedding
        X = nn.utils.rnn.pack_padded_sequence(input=X, lengths=source_lengths, batch_first=False)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens, _ = nn.utils.rnn.pad_packed_sequence(enc_hiddens, batch_first=False) # sentence_L x batch_size x hidden_size * 2
        hiddens_1 = self.project_1(enc_hiddens) # sentence_L x batch_size x word_embedding

        # layer 2
        hiddens_1_packed = nn.utils.rnn.pack_padded_sequence(input=hiddens_1, lengths=source_lengths, batch_first=False)
        enc_hiddens, (last_hidden, last_cell) = self.encoder_2(hiddens_1_packed)
        enc_hiddens, _ = nn.utils.rnn.pad_packed_sequence(enc_hiddens, batch_first=False)
        hiddens_2 = self.project_2(enc_hiddens)
        hiddens_2 = self.connect_2(hiddens_2, hiddens_1)

        #layer3
        hiddens_2_packed = nn.utils.rnn.pack_padded_sequence(input=hiddens_2, lengths=source_lengths, batch_first=False)
        enc_hiddens, (last_hidden, last_cell) = self.encoder_3(hiddens_2_packed)
        enc_hiddens, _ = nn.utils.rnn.pad_packed_sequence(enc_hiddens, batch_first=False)
        hiddens_3 = self.project_3(enc_hiddens)
        hiddens_3 = self.connect_3(hiddens_3, hiddens_2)

        enc_hiddens = hiddens_3.permute(1, 0, 2)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int = 5, max_decoding_time_step: int = 70) -> List[
        ...
        if self.multi_encoder:
            src_encodings, dec_init_vec = self.multi_encode(src_sents_var, [len(src_sent)])
        else: 
    ...
    @staticmethod
    def load(model_path: str, no_char_decoder=False, with_contex=False, contex_LSTM=False, 
        multi_encoder=False):
        ...
        args = params['args']
        model = NMT(vocab=params['vocab'], no_char_decoder=no_char_decoder, with_contex=with_contex,
            contex_LSTM=contex_LSTM, multi_encoder=multi_encoder, **args)
