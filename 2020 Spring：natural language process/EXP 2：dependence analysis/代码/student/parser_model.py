...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.zeros([self.hidden_size, n_features * self.embed_size]))
        self.embed_to_hidden_bias = nn.Parameter(torch.zeros([self.hidden_size]))
        self.relu = nn.ReLU()
        self.hidden_to_logits_weight = nn.Parameter(torch.zeros([self.n_classes, self.hidden_size]))
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.stack([torch.index_select(self.embeddings, 0, idx) for idx in w], dim=0)
    ...
    def forward(self, w):
        ...
        hidden = x.matmul(self.embed_to_hidden_weight.t()) + self.embed_to_hidden_bias
        hidden = self.relu(hidden)
        hidden_to_logits_weight = self.dropout(self.hidden_to_logits_weight)
        logits = hidden.matmul(hidden_to_logits_weight.t()) + self.hidden_to_logits_bias
        ### END YOUR CODE
        return logits
